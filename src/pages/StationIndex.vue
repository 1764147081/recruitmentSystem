					...
					<el-upload action="#" :auto-upload="false" :on-change="handleFileChange" :show-file-list="false"
						...
						<div class="upload-square">
							...
							<div v-if="formData.image" class="image-actions">
								<el-button type="danger" icon="el-icon-delete" @click.stop="handleRemoveImage" circle
	...
	<div class="editForm" v-if="ifEdit">
		<el-form :model="editForm" label-width="120px" class="demo-ruleForm">
			...
			<el-form-item label="上传部门海报" prop="image" v-if="station.getIsDepartment == 1">
				<el-upload action="#" :auto-upload="false" :on-change="handleEditFileChange" :show-file-list="false"
					accept="image/*">
					<div class="upload-square">
						...
						<div v-if="editForm.image" class="image-actions">
							<el-button type="danger" icon="el-icon-delete" @click.stop="handleRemoveEditImage" circle
								size="small"></el-button>
						</div>
					</div>
					<div class="upload-tip">请选择图片文件（支持 JPG/PNG）</div>
	...
	async function search() {
		...
		try {
			...
			if (data.code == 200) {
				...
				alert(`已查询到对应人员,${data.data.name},可点击对应部门添加其作为管理员`)
				console.log(data);
			}
		} catch (error) {
			console.log(error)
